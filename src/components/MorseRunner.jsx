...
export const MorseRunner = ({
  ...
  const generateNewQso = useCallback(() => {
  ...
  const playSequence = (sequence) => {
    ...
    try {
      ...
      // Use the actual sendDelay from props
      const currentSendDelay = sendDelay || 0.5;
      ...
      }, currentSendDelay * 1000); // Convert to milliseconds
    } catch (error) {
  ...
  const submitExchange = () => {
    ...
    if (isCorrect) {
      ...
      // Start the next QSO after delay - Use qsoRate from props
      qsoTimeoutRef.current = setTimeout(() => {
        if (running && newCallsign) {
          playSequence(newCallsign);
        }
      }, qsoRate * 1000); // Convert to milliseconds
    } else {
      // Show error notification
